...
# Wrap all logic in a function so the script always exits 0 regardless of
# any internal failure. set -e is intentionally NOT used here because this
# script runs as a PreToolUse hook — a non-zero exit would block the tool call.
...
' 2>/dev/null || echo "")
  ...
  SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)" || return 0
  SKILL_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)" || return 0

  # Source shared project detection helper
  # This sets: PROJECT_ID, PROJECT_NAME, PROJECT_ROOT, PROJECT_DIR
  source "${SKILL_ROOT}/scripts/detect-project.sh" 2>/dev/null || return 0
...
" >> "$OBSERVATIONS_FILE" 2>/dev/null || true
  ...
  for pid_file in "${PROJECT_DIR}/.observer.pid" "${CONFIG_DIR}/.observer.pid"; do
    if [ -f "$pid_file" ]; then
      observer_pid=$(cat "$pid_file" 2>/dev/null) || continue
...
# Run main logic; guarantee exit 0 even if the function itself fails
_observe_main "$@" || true
